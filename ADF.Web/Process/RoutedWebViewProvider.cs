...
namespace Adf.Web.Process
{
    ...
    /// </summary>
    public class RoutedWebViewProvider : IViewProvider
    {
        public RoutedWebViewProvider()
        {
            // Setting up routes for use cases
            RouteTable.Routes.MapPageRoute("Tasks", "Task/{task}/{id}", "~/Forms/{task}.aspx");
        }
        ...
        {
            string page = string.Format(CultureInfo.InvariantCulture, "~/Task/{0}/{1}", task.Name, task.Id);
